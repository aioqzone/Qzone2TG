...
class Key(Compress):
    ...
    @staticmethod
    def frombytes(b: bytes):
        return Key('{:x}'.format(int.from_bytes(b, byteorder = sys.byteorder, signed = False)))

class MoodUrl(Compress):
    uin: int
    key: str
    def __init__(self, uin: str, key: str):
        self.uin = int(uin); self.key = key
    
    def tobytes(self):
        return self.uin.to_bytes(5, sys.byteorder, signed = False) + Key(self.key).tobytes()
    @staticmethod
    def frombytes(b: bytes):
        assert len(b) == 17
        return MoodUrl(
...
class LikeId(Compress):
    ...
    def frombytes(b: dict):
        ...
        return LikeId(
            ids.appid, ids.typeid, Key.frombytes(b[2: 14]).key, 
            url.format(uin = uni.uin, key = uni.key), 
