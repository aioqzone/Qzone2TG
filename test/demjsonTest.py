...
class AccTest(TestCase):
    def testAcc(self):
        m = json_loads(js)
        d = demjson.decode(js)
        self.assertListEqual(list(m.keys()), list(d.keys()))

    def testExc(self):
        self.assertRaises(SyntaxError, json_loads, js.replace('}', '', 1))
        self.assertRaises(SyntaxError, json_loads, js.replace('"', '', 1))
...
class TestSpeed(TestCase):
    ...
    def testDemjson(self):
        t = time()
        self.assertTrue(demjson.decode(js))
        t -= time()
        print(-t, 's')

    def testRegulate(self):
        t = time()
        self.assertTrue(json_loads(js))
        t -= time()
        print(-t, 's')
