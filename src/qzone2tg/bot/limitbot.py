...
class SemaBot:
    ...
    async def send_video(self, to: Union[int, str], text: str, media: HttpUrl, **kw):
        assert len(text) < MEDIA_TEXT_LIM
        kwds = dict(chat_id=to, caption=text, video=str(media))
        async with self.sem.num():
            return self.bot.send_video(**kwds, **kw)

    async def send_media_group(self, to: Union[int, str], media: list[InputMedia], **kw):
...
class LimitedBot(SemaBot):
    ...
    async def unify_send(self, to: Union[int, str], text: str, media: list[HttpUrl], **kw):
        if not media:
